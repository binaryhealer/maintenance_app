...
if not existing_service_log:
    ...
    service_log.custom_notes = doc.custom_work_progress or doc.get("custom_description") or ""
    ...
    if not existing_lifecycle:
        ...
        if doc.custom_asset:
            asset_values = frappe.db.get_value(
                "Installed Equipment",
                doc.custom_asset,
                ["custom_make", "custom_model", "custom_branch", "custom_asset_id"],
                as_dict=True
            )
...
# =========================
# UPDATE INSTALLED EQUIPMENT AFTER RELOCATION
# =========================
...
if str(doc.get("custom_component_replaced")) == "1":
    ...
    # Lifecycle Log for component replacement
    comp_lifecycle_log = frappe.new_doc("Asset Lifecycle Log")

    comp_lifecycle_log.custom_customer = doc.custom_parent_customer
    comp_lifecycle_log.custom_branch = doc.custom_branch
    comp_lifecycle_log.custom_related_asset = doc.custom_asset

    comp_lifecycle_log.custom_date = doc.custom_start_time or frappe.utils.now_datetime()
    comp_lifecycle_log.custom_event = "Component Replaced"
    comp_lifecycle_log.custom_event_source = "Intervention"

    comp_lifecycle_log.custom_action_taken = (
        doc.get("custom_component_replacement_reason")
        or "Component Replaced"
    )

    comp_lifecycle_log.custom_technician = doc.custom_technician or frappe.session.user
    comp_lifecycle_log.custom_service_ticket = doc.custom_service_ticket
    comp_lifecycle_log.custom_mission_ref = doc.custom_parent_mission
    comp_lifecycle_log.custom_intervention_ref = doc.name

    # Old component details
    comp_lifecycle_log.custom_component_item = doc.get("custom_component_item")
    comp_lifecycle_log.custom_component_serial = doc.get("custom_component_serial")
    comp_lifecycle_log.custom_component_brand = doc.get("custom_component_brand")
    comp_lifecycle_log.custom_component_model = doc.get("custom_component_model")

    # New component details
    comp_lifecycle_log.custom_new_component_item = doc.get("custom_new_component_item")
    comp_lifecycle_log.custom_new_component_serial = doc.get("custom_new_component_serial")
    comp_lifecycle_log.custom_new_component_brand = doc.get("custom_new_component_brand")
    comp_lifecycle_log.custom_new_component_model = doc.get("custom_new_component_model")

    comp_lifecycle_log.insert(ignore_permissions=True)

    if comp_lifecycle_log.docstatus == 0:
        comp_lifecycle_log.submit()
...
if doc.get("custom_parent_mission"):
    ...
    if mission.get("custom_service_ticket"):
        ...
        if outcome == "Work Completed":
            ...
            ticket.custom_ticket_status = "On Hold"
            ticket.custom_hold_reason = "Awaiting Client"

        elif outcome == "Continue Work":
            ticket.custom_ticket_status = "On Hold"
            ticket.custom_hold_reason = "Awaiting Internal Review"

        elif outcome == "Could Not Complete":
            ticket.custom_ticket_status = "On Hold"
            ticket.custom_hold_reason = "Other"
