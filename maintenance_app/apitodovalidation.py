...
if planning.get("custom_planned_starttime"):
    planned_date = getdate(
        planning.get("custom_planned_starttime")
    )

for tech in planning.get("custom_planned_team") or []:

    technician = tech.get("custom_technician")

    if not technician or not planned_date:
        continue
    ...
    if existing_todo:
        ...
        frappe.msgprint(
            f"Warning: Technician {technician} already has an open task on {planned_date}."
        )
...
def create_intervention_from_mission(mission_name):
    ...
    mi.custom_intervention_type = mission.custom_ticket_type or "Service"
