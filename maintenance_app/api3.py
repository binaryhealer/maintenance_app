...
def _create_todos_for_mission(mission, planning):
    ...
    for row in planning.get("custom_planned_team") or []:
        ...
        if not technician:
            skipped.append("Blank technician row")
            continue
        ...
        if not allocated_to:
            ...
            frappe.log_error(
                f"Cannot create ToDo. Technician '{technician}' is not a valid User and is not linked to an Employee user_id.",
        ...
        todo.date = getdate(planning.get("custom_planned_starttime"))
    ...
    if created == 0:
        frappe.msgprint(
            ...
            "Check Planned Technicians: technician must be a User or Employee linked to a User ID."
        )

    if skipped:
        frappe.log_error(
            "Skipped ToDo technicians: " + ", ".join(skipped),
            "Tech Mission ToDo Skipped"
        )
